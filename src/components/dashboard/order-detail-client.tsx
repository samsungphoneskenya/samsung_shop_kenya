...
export function OrderDetailClient({ order, items }: OrderDetailClientProps) {
  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          variant: "success",
          title: "WhatsApp marked as sent",
          description: "Timestamp recorded.",
        });
        router.refresh();
      }
    } finally {
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      router.push(result.redirectTo ?? "/dashboard/orders");
    } finally {
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div className="pt-2">
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                <p className="mt-2 text-xs text-gray-500">
                  This permanently deletes the order and its items.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-6 text-sm">
          ...
          {Number(order.shipping_fee || 0) > 0 && (
            <span>
              Shipping: KSh {Number(order.shipping_fee).toLocaleString()}
            </span>
          )}
